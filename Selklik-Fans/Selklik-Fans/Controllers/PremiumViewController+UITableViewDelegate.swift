//
//  PremiumViewController+UITableViewDelegate.swift
//  Selklik-Fans
//
...
extension PremiumViewController:UITableViewDelegate {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!) {
        if (segue.identifier == "PremiumToViewPhoto") {
        ...
        if (segue.identifier == "PremiumToPlayVideo") {
            let destination = segue.destinationViewController as! AVPlayerViewController
            let url = NSURL(string:selectedVideoUrl!)
            destination.player = AVPlayer(URL: url!)
        }

        if (segue.identifier == "PremiumToComment") {
